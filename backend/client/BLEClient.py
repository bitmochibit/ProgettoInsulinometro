...
from backend.device.ble.Scanner import Scanner
...
class BLEClient(Client):
	...
	def read_data(self, device_property: DeviceProperty, callback: Callable[[Any, Any], None] = None):
		...
		asyncio.run_coroutine_threadsafe(self._read_data(device_property, callback), self.loop)
	...
	def write_data(self, device_property: DeviceProperty, data: Any, callback: Callable[[Any, Any], None] = None):
		...
		asyncio.run_coroutine_threadsafe(self._write_data(device_property, data, callback), self.loop)

	# Internal handling methods

	async def _read_data(self, device_property: DeviceProperty, callback: Callable[[Any, Any], None] = None):
	...
	async def _write_data(self, device_property: DeviceProperty, data: Any, callback: Callable[[Any, Any], None] = None):
		...
		try:
			...
			if callback:
				callback(True, None)  # Pass success to callback
		except BleakError as e:
			print(f"Failed to write data: {e}")
			if callback:
				callback(False, e)  # Pass failure to callback
