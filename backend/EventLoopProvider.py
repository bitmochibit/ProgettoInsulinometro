...
class EventLoopProvider:
	...
	def get_event_loop():
		...
		if EventLoopProvider._loop_instance is None:
			EventLoopProvider().start_event_loop()

		return EventLoopProvider._loop_instance
	...
	def run_async(coro, callback: Callable[[Any, Any], None] = None, timeout = 100):
		...
		future = asyncio.run_coroutine_threadsafe(with_timeout(), EventLoopProvider.get_event_loop())
		if callback:
	...
	def start_event_loop(self):
		...
		if self._loop_instance and self._loop_instance.is_running():
			...
			return
		EventLoopProvider._loop_instance = asyncio.new_event_loop()
		EventLoopProvider._loop_thread = threading.Thread(target=self._loop_instance.run_forever, daemon=True)
		EventLoopProvider._loop_thread.start()
	...
	def stop_event_loop(self):
		...
		if self._loop_instance and self._loop_instance.is_running():
			...
			EventLoopProvider._loop_instance = None
